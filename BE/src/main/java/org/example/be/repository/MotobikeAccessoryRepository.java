...
public interface MotobikeAccessoryRepository extends JpaRepository<MotobikeAccessory, Long> {
    @Query(value = "select * from motobike_accessory\n" +
            "order by motobike_accessory.date desc limit 10", nativeQuery = true)
    ...
    @Query(value = "select * from motobike_accessory where motobike_accessory.name like :name ", nativeQuery = true)
